...
class VinaDock:
    ...
    def get_docking_box(self, pdb_file):
        """
        ...
        # 计算并四舍五入对接框的中心坐标
        center = np.round((min_vals + max_vals) / 2, 6)
        # 计算并四舍五入对接框的尺寸
        size = np.round(max_vals - min_vals, 6)
    ...
    def split_protein_ligand(self, pdb_file):
        # 读取PDB文件
        mol = Chem.MolFromPDBFile(pdb_file, sanitize=False, removeHs=False)
        if mol is None:
            raise ValueError("无法读取PDB文件")
    ...
    def run_lepro(self, complex_pdb: str, out_pdb_file: str):
        """
    ...
    def docking(self, ligand_file):
        ...
        log.info(f"complex file: {complex_file}")
        center, size = self.get_docking_box(complex_file)
        log.info(f"box center coordinate: {center}")
        log.info(f"box size coordinate: {size}")

        self.run_lepro(complex_file, protein_file)

        self.pdb_to_pdbqt(protein_file, protein_pdbqt)
